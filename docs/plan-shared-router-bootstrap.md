...
- Generated wiring: the emitters need the binding registration APIs to
  exist first. The slice's hand-written test glue IS the draft of what
  `angzarr codegen <lang>` will emit — the same forcing function the
  Go consolidation used.
...
One fixture component, specified in `FIXTURE.md` prose and implemented
in ~40 lines per language:
...
- The registration API is shaped like the engine's
  ...
  will eventually emit against it with minimal emitter changes.
...
Each unit is one reviewable change. After unit 5: **ABI freeze review**
...
## 6. What this defers, explicitly

- No change to what client-go ships today — the engine remains the
  dispatch surface until R3's parity gate.
- No emitter changes in angzarr-cli yet; the hand-written fixture glue
  in units 4–5 is the draft the emitters will codify.
